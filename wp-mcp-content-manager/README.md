...
> **WP MCP Server By AZ** is a free, open-source WordPress plugin that turns any site into a secure **Model Context Protocol (MCP)** server. Connect [Claude](https://claude.ai) — or any MCP-compatible AI agent — and read, create, and update **pages, posts, custom post types, ACF fields, media, taxonomies, and SEO meta** using natural language, one item at a time or in bulk.
